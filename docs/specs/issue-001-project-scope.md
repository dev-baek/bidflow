...
11. EventBridge Scheduler 기반 경매 종료
12. 낙찰·유찰 결과 확정
13. Consumer 재시도와 실패 이벤트 격리
14. Redis 상태 복구와 OpenSearch 전체 재색인
15. CloudWatch 로그, 지표, Dashboard와 Alarm
16. Terraform 기반 AWS 검증 환경 생성·삭제
17. 동시 입찰 부하 테스트와 장애 복구 실험

### 5.2 시간이 남으면 구현

- 종료 직전 입찰 시 경매 시간 자동 연장
- 상품 검색 자동완성
- 관심 상품
- 이메일 알림
- 판매자 통계 화면
...
7. `BidAccepted` 또는 `BidRejected` 이벤트를 발행한다.
8. WebSocket Consumer가 사용자에게 최종 결과를 전송한다.
...
| 상품 상세·입찰 | 연결 중, 입찰 접수, 처리 대기, 승인, 거절, 연결 끊김, 종료 |
